...
async function main(client) {
  ...
  response = await deauditAcc.runLocal("withdrawOpenStatus", {});
  console.log("Contract reacted to your withdrawOpenStatus:", response.decoded.output);

  response = await deauditAcc.runLocal("totalStaked", {});
  console.log("Contract reacted to your totalStaked:", parseInt(response.decoded.output.totalStaked));

  response = await deauditAcc.runLocal("getBalance", {_answer_id:0});
  ...
  console.log("DeAudit name:", hex2ascii(response.decoded.output.name));
  ...
  console.log("===> DeAudit Candidates votes accumulating list:");

  for (const item of candidateKeys) {
    console.log(hex2ascii(candidate[item].name));
    console.log(parseInt(candidate[item].votes));
    ...
    for (const item1 of mbkeys) {
      ...
      for (const item2 of vpkeys) {
        ...
        for (const item3 of vckeys) {
          ...
          let votesArr = vc[item3].votes;
          let cindex = 0
          for (const candidateVotes of votesArr) {
            console.log ('candidate index: '+cindex);
            console.log('votes: '+parseInt(candidateVotes));
            cindex ++;
          }
          // console.log('votes: '+vc[item3].votes);
          console.log('collationStatus: '+vc[item3].collationStatus);
          // console.log('act4Arr: '+vc[item3].act4Arr);
          let act4Arr = vc[item3].act4Arr;
          let validators = vc[item3].validatorsArr;
          for (const act4 of act4Arr) {
            console.log('act4: '+act4);
            console.log('validtor arr: '+validators[act4]);
          }
          // console.log('validatorsArr: '+vc[item3].validatorsArr);
          console.log('**************');
        }
      }
    }
  ...
  // let district = response.decoded.output.district;
  console.log("===> DeAudit Districts votes accumulating list:");

  for (const item of districtKeys) {
    console.log(hex2ascii(district[item].name));
    // console.log(district[item].votes);
    let voteArr = district[item].votes;
    let candidateIndex = 0;
    for (const candidateVotes of voteArr) {
      let candidateKey = candidateKeys[candidateIndex];
      console.log(hex2ascii(candidate[candidateKey].name)+' : '+parseInt(candidateVotes));
      candidateIndex ++;
    }
  ...
  console.log("===> DeAudit Municipal Bodies votes accumulating list:");

  for (const item of municipalBodyKeys) {
    console.log(hex2ascii(municipalBody[item].name));
    // console.log(municipalBody[item].votes);

    let voteArr = municipalBody[item].votes;
    let candidateIndex = 0;
    for (const candidateVotes of voteArr) {
      let candidateKey = candidateKeys[candidateIndex];
      console.log(hex2ascii(candidate[candidateKey].name)+' : '+parseInt(candidateVotes));
  ...
  console.log("===> DeAudit Voting pools votes accumulating list:");

  for (const item of votingPoolKeys) {
    console.log(hex2ascii(votingPool[item].name));
    // console.log(votingPool[item].votes);

    let voteArr = votingPool[item].votes;
    let candidateIndex = 0;
    for (const candidateVotes of voteArr) {
      let candidateKey = candidateKeys[candidateIndex];
      console.log(hex2ascii(candidate[candidateKey].name)+' : '+parseInt(candidateVotes));
  ...
  console.log("===> DeAudi Voting centeres votes accumulating list:");

  for (const item of votingCenterKeysArr) {
    console.log(hex2ascii(votingCenterKeysObj[item].name));
    // console.log(votingCenterKeysObj[item].votes);

    let voteArr = votingCenterKeysObj[item].votes;
    let candidateIndex = 0;
    for (const candidateVotes of voteArr) {
      let candidateKey = candidateKeys[candidateIndex];
      console.log(hex2ascii(candidate[candidateKey].name)+' : '+parseInt(candidateVotes));
