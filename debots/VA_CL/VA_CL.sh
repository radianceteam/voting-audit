...
COREDEBOTADDR=$(cat ../VA_CR/address.log)

echo DEPLOY DEBOT $DEBOT_ADDRESS
$TOS --url $NETWORK deploy $DEBOT_NAME.tvc "{}" --sign $DEBOT_NAME.keys.json --abi $DEBOT_NAME.abi.json
$TOS --url $NETWORK call $DEBOT_ADDRESS setABI "{\"dabi\":\"$DEBOT_ABI\",\"coreDebot\":\"$COREDEBOTADDR\"}" --sign $DEBOT_NAME.keys.json --abi $DEBOT_NAME.abi.json
