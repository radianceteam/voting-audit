...
address curACT4adrACT;
    function onGetCollatorPhotoLink(uint32 index) public {
        ...
        Terminal.print(0,format("Selected Act4 address: {}\n",curACT4adrACT));
    ...
    function DAmenu(uint32 index) public {
        ...
        for(uint8 i = 0; i < keysDeAuditD.length; i++){
            ...
            uint256 time = uint256(now);
                ...
                string curVdata = format("{}\nValidation stake: {} nanoTONs\nStatus:{}\n",cp.name,cp.valStake,status);
...
bytes nameDD;
    ...
    function showVotingAuditss(uint32 index) public {
        ...
        if(time > (cr.timeStart + cr.colPeriod) && time < (cr.timeStart + cr.colPeriod + cr.valPeriod)){
            AmountInput.get(tvm.functionId(setValStake), format("Validation stake is: {} nanoTONs for 1 validation, 0.5 ton should be added as registration fee\n",valSt),0,0,10000000000000);
        }else{
            Terminal.print(0,"You need to choose available for Validation DeAudit");
            DAmenu(0);
        }
    }
...
uint128 curGramsForSend;
    function setValStake(uint128 value) public {
        ...
        Terminal.print(0,format("You are going to take part in <{}>, it`s address: {}, stake for 1 validation is {} nanoTONs, you are going to send {} nanoTONs\n", nameDD, cureDA, valSt,curGramsForSend));
    setValStakeCall();
    }
